...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.pages.asset_exchange = function (callback) {
        ...
        NRS.storageSelect("assets", null, function (error, assets) {
            ...
            $.get("/das?=%2Fdas&requestType=getAssetIds", '', function (data) {
                var response = JSON.parse(data);
                if(response && response.assetIds && response.assetIds.length > 0){
                    var dasAssetIds = response.assetIds;
                    for (var index = 0; index < dasAssetIds.length; index++) {
                        var dasAssetIdFlag = true;
    
                        for (let i = 0; i < assets.length; i++) {
                            if(assets[i].asset === dasAssetIds[index]){
                                dasAssetIdFlag = false;
                                break
                            }
                        }
                        
                        if(dasAssetIdFlag){
                            var url = "/das?=%2Fdas&requestType=getAsset&asset=" + dasAssetIds[index];
                            $.get(url, '', function (responseDetail) {  
                                var assetDetail = JSON.parse(responseDetail);
                                console.log("assetDetail ", assetDetail)
                                var html = '<option data-select2-name="'+ assetDetail.description +'" value="' + assetDetail.account + '">' + assetDetail.name + '</option>';
                                $("#assetIdSelections").append(html);
                            });
                        }
                    }
                }
    ...
    NRS.forms.addAssetBookmark = function ($modal) {
        var selectedAssets = $("#assetIdSelections").val();
        var data = {
            id:""
        }
        ...
        for (let index = 0; index < selectedAssets.length; index++) {
            ...
            if (!/^\d+$/.test(data.id) && !/^DAS\-/i.test(data.id)) {
                return {
                    "error": $.t("error_asset_or_account_id_invalid")
                };
            }
    
            if (/^DAS\-/i.test(data.id)) {
    ...
    NRS.loadAssetExchangeSidebar = function (callback) {
        ...
        for (var index = 0; index < assets.length; index++) {
            if(assetsTemporary.length > 0){
                var flag = true;
                for (var j = 0; j < assetsTemporary.length; j++) {
                    if(assetsTemporary[j].asset == assets[index].asset){
                        flag = false;
                        break;
                    }                    
                }
                ...
                if(flag){
                    assetsTemporary.push(assets[index])
                }
            }else{
                assetsTemporary.push(assets[index])
            }
        }
        assets = assetsTemporary;
        ...
        for (var i = 0; i < assets.length; i++) {
            ...
            rows += "<h4 class='list-group-item-heading text-uppercase'>" + NRS.escapeRespStr(asset.name) + "</h4>";
        ...
        console.log("assetExchangeSidebarContent ", rows);
    ...
    NRS.loadAsset = function (asset, refresh) {
        ...
        if (!refresh) {
            ...
            $("#sell_asset_for_das").html($.t("sell_asset_for_das", {
                "assetName": NRS.escapeRespStr(asset.name)
            }));
            $("#buy_asset_with_das").html($.t("buy_asset_with_das", {
                "assetName": NRS.escapeRespStr(asset.name)
            }));
        ...
        if (NRS.accountInfo.unconfirmedBalanceNQT == "0") {
            $("#your_das_balance").html("0");
            $("#buy_automatic_price").addClass("zero").removeClass("nonzero");
        } else {
            $("#your_das_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceNQT));
    ...
    $("#asset_exchange_bid_orders_table tbody, #asset_exchange_ask_orders_table tbody").on("click", "td", function (e) {
        ...
        try {
            ...
            $("#" + type + "_asset_total").val(NRS.convertToDAS(totalNQT));
        } catch (err) {
            return;
        }
    ...
    $("#sell_automatic_price, #buy_automatic_price").on("click", function () {
        try {
            ...
            if (price.cmp(new Big("0")) <= 0) {
                ...
                assetPrice.val(NRS.convertToDAS(price.toString()));
            }
            ...
            assetTotal.val(NRS.convertToDAS(total.toString()));
            assetTotal.css({
                "background": "",
                "color": ""
            });
    ...
    $("#asset_order_modal").on("show.bs.modal", function (e) {
        ...
        try {
            ...
            var priceNQT = new BigInteger(NRS.calculatePricePerWholeQNT(NRS.convertToNQT(String($("#" + orderType + "_asset_price").val())), currentAsset.decimals));
            var totalDAS = NRS.formatAmount(NRS.calculateOrderTotalNQT(quantityQNT, priceNQT, currentAsset.decimals), false, true);
        } catch (err) {
        ...
        if (orderType == "buy") {
            description = $.t("buy_order_description", {
                ...
                "das": NRS.formatAmount(priceNQTPerWholeQNT)
            });
            tooltipTitle = $.t("buy_order_description_help", {
                "das": NRS.formatAmount(priceNQTPerWholeQNT, false, true),
                "total_das": totalDAS
            });
        } else {
        ...
        $("#asset_order_total").html(totalDAS + " DAS");
    ...
    NRS.forms.issueAsset = function ($modal) {
        $.get("/das?=%2Fdas&requestType=getAssetIds", '', function (data) {
            ...
            if(response && response.assetIds && response.assetIds.length > 0){
                var dasAssetIds = response.assetIds;
                $("#assetIdSelections").empty();
                for (var index = 0; index < dasAssetIds.length; index++) {
                    var url = "/das?=%2Fdas&requestType=getAsset&asset=" + dasAssetIds[index];
                    $.get(url, '', function (responseDetail) {  
                        var assetDetail = JSON.parse(responseDetail);
                        var html = '<option data-select2-name="'+ assetDetail.description +'" value="' + assetDetail.asset + '">' + assetDetail.name + '</option>';
                        $("#assetIdSelections").append(html);
                    });        
                }
            }
    ...
    function formatState (state) {
        ...
        var $state = $(
            '<span class="text-left">' + state.text + '</span>' + '<span class="float-right">' + state.element.attributes[0].value + '</span>'
        );
        return $state;
    };
    ...
    NRS.setup.asset_exchange = function () {
        ...
        // NRS.appendMenuItemToTSMenuItem(sidebarId, options);
        options = {
			"titleHTML": '<span data-i18n="send_das">Transfer DAS</span>',
			"type": 'MODAL',
