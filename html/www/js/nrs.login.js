...
var NRS = (function (NRS, $, undefined) {
	...
	NRS.showLoginOrWelcomeScreen = function () {
		$("#logo-das-img").parent().removeClass("d-none");
	...
	NRS.showLoginScreen = function () {
		$("#account_phrase_custom_panel, #account_phrase_generator_panel, #welcome_panel").hide();
	...
	NRS.showWelcomeScreen = function () {
		$("#login_panel, #account_phrase_generator_panel, #account_phrase_custom_panel, #welcome_panel").hide();
		if (NRS.isMobileApp()) {
			$(".mobile-only").show();
		}
		$("#welcome_panel").show();
	...
	NRS.registerUserDefinedAccount = function () {
		$("#account_phrase_generator_panel, #login_panel, #welcome_panel").hide();
		$("#account_phrase_generator_panel").find(":input:not(:button):not([type=submit])").val("");
		$("#logo-das-img").parent().addClass("d-none");
	...
	NRS.registerAccount = function () {
		$("#login_panel, #welcome_panel, #account_phrase_custom_panel").hide();
		$("#logo-das-img").parent().removeClass("d-none");
	...
	NRS.verifyGeneratedPassphrase = function () {
		console.log("verifyGeneratedPassphrase");
	...
	NRS.listAccounts = function () {
		...
		loginAccount.empty();
		if (NRS.getStrItem("savedDasAccounts") && NRS.getStrItem("savedDasAccounts") != "") {
			$('#login_account_container').show();
			// $('#login_account_container_other').hide();
			var accounts = NRS.getStrItem("savedDasAccounts").split(";");
	...
	$("#loginButtons").find(".btn").click(function (e) {
		...
		if (type == "account") {
			...
			NRS.scanQRCode(readerId, function (text) {
				var dasAddress = new DasAddress();
				if (dasAddress.set(text)) {
	...
	NRS.removeAccount = function (account) {
		var accounts = NRS.getStrItem("savedDasAccounts").replace(account + ';', '');
		if (accounts == '') {
			NRS.removeItem('savedDasAccounts');
		} else {
			NRS.setStrItem("savedDasAccounts", accounts);
		}
		NRS.listAccounts();
	};

	function rememberAccount(account) {
		var accountsStr = NRS.getStrItem("savedDasAccounts");
		if (!accountsStr) {
			NRS.setStrItem("savedDasAccounts", account + ";");
			return;
		}
		...
		NRS.setStrItem("savedDasAccounts", accountsStr + account + ";");
	}
		...
		NRS.sendRequest("getBlockchainStatus", {}, function (response) {
			...
			NRS.sendRequest(accountRequest, requestVariable, function (response, data) {
				...
				NRS.sendRequest("getAccountPublicKey", {
					...
					$("#sidebar_account_link").html(NRS.getAccountLink(NRS, "account", NRS.accountRS, "details", false, "btn btn-primary-r5 btn-xs"));
					...
					if (NRS.getStrItem("savedDasAccounts") && NRS.getStrItem("savedDasAccounts") != "") {
						...
						accounts = NRS.getStrItem("savedDasAccounts").split(";");
						$.each(accounts, function (index, account) {
							if (account != '') {
	...
	NRS.initPluginWarning = function () {
		if (NRS.activePlugins) {
			...
			html += "Plugins are not sandboxed or restricted in any way and have full accesss to your client system including your Das passphrase.";
			html += "</p>";
			...
			html += "you can loose your DAS! In doubt don't run plugins with accounts ";
			html += "used to store larger amounts of DAS now or in the future.";
			html += "</p>";
			html += "</div>";
