...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.initAdvancedModalFormValues = function($modal) {
		...
		context = {
			...
			inputName: "phasingQuorumDAS",
			addonText: "DAS",
			addonI18n: "das_unit"
		};
		$elems = NRS.initModalUIElement($modal, '.approve_transaction_amount_das', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context['inputName'] = 'phasingMinBalanceDAS';
		context['addonText'] = 'DAS';
		context['addonI18n'] = 'das_unit';
		$elems = NRS.initModalUIElement($modal, '.approve_min_balance_das', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
