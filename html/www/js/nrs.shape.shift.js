...
var NRS = (function(NRS, $) {
    ...
    var coinToPair = function (op, coin) {
        return (op == "buy") ? "DAS_" + coin : coin + "_DAS";
    };

    var pairToCoin = function (pair) {
        if (pair.indexOf("DAS_") == 0) {
            return pair.substring("DAS_".length);
        }
        if (pair.indexOf("_DAS") == pair.length - "_DAS".length) {
            return pair.substring(0, pair.indexOf("_DAS"));
        }
    ...
    var renderExchangeTable = function (op) {
        ...
        async.series(tasks, function (err, results) {
            var table = $("#p_shape_shift_" + op + "_das");
            if (err) {
        ...
        apiCall('recenttx/50', {}, 'GET', function (data) {
            ...
            if (data) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (String(transaction.curIn).escapeHTML() != "DAS" && String(transaction.curOut).escapeHTML() != "DAS") {
                        continue;
                    }
                    rows += "<tr>";
    ...
    function renderDasLimit() {
        apiCall('limit/das_btc', {}, 'GET', function (data) {
            NRS.logConsole("limit1 " + data.limit);
            if (data.limit) {
                $('#shape_shift_status').html('ok');
                $('#shape_shift_das_avail').html(String(data.limit).escapeHTML());
            }
        });
    }
        ...
        apiCall('getcoins', {}, 'GET', function (data) {
            ...
            for (var i = 0; i < inputFields.length; i++) {
                ...
                $.each(data, function (code, coin) {
                    if (code != 'DAS' && coin['status'] == 'available') {
    ...
    NRS.getFundAccountLink = function() {
        ...
        //     "data-pair='BTC_DAS'>" + $.t("fund_account_message") + "</a>";
        return "";
    };
    ...
    $("#m_shape_shift_buy_modal").on("show.bs.modal", function (e) {
        ...
        $("#m_shape_shift_buy_title").html($.t("exchange_das_to_coin_shift", { coin: coin }));
        $("#m_shape_shift_buy_min").val(invoker.data("min"));
        $("#m_shape_shift_buy_min_coin").html("DAS");
        $("#m_shape_shift_buy_max").val(invoker.data("max"));
        $("#m_shape_shift_buy_max_coin").html("DAS");
        $("#m_shape_shift_buy_rate").val(invoker.data("rate"));
        $("#m_shape_shift_buy_rate_text").html("DAS/" + coin);
    ...
    $("#m_send_amount_buy_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_buy_title").html($.t("exchange_das_to_coin_send_amount", { coin: coin }));
        $("#m_send_amount_buy_withdrawal_amount_coin").html(coin);
        $("#m_send_amount_buy_rate_text").html("DAS/" + coin);
    ...
    $('#m_send_amount_buy_withdrawal_amount, #m_send_amount_buy_withdrawal_address').change(function () {
        ...
        apiCall('sendamount', {
            ...
            try {
                ...
                // add 1 DAS fee to make sure the net amount is what requested by shape shift
    ...
    $("#m_shape_shift_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_shape_shift_sell_title").html($.t("exchange_coin_to_das_shift", { coin: coin }));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                $("#m_shape_shift_sell_rate_text").html(coin + "/DAS");
                $("#m_shape_shift_sell_fee").val(data.fee);
                $("#m_shape_shift_sell_fee_coin").html("DAS");
    ...
    $("#m_send_amount_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_sell_title").html($.t("exchange_coin_to_das_send_amount", { coin: coin }));
        $("#m_send_amount_sell_rate_text").html("DAS/" + coin);
        $("#m_send_amount_sell_fee_coin").html("DAS");
        $("#m_send_amount_sell_withdrawal_amount_coin").html("DAS");
