...
var NRS = (function (NRS, $, undefined) {
    ...
    search.on("submit", function (e, data) {
        ...
        $("#buy_currency_with_das").html($.t("buy_currency_param", { currency: currencyCode}));
        $("#buy_currency_offers").html($.t("offers_to_buy_currency_param", { currency: currencyCode}));
        $("#sell_currency_with_das").html($.t("sell_currency_param", { currency: currencyCode}));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                if (NRS.accountInfo.unconfirmedBalanceNQT == "0") {
                    $("#ms_your_das_balance").html("0");
                } else {
                    $("#ms_your_das_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceNQT));
                }
                NRS.pageLoaded();
                callback(null);
            }
        ...
        NRS.sendRequest("getCurrencyFounders", {
            ...
            if (response.founders && response.founders.length) {
                ...
                for (i = 0; i < response.founders.length; i++) {
                    ...
                    "<td>" + NRS.convertToDAS(amountPerUnitNQT) + "</td>" +
                    "<td>" + NRS.convertToDAS(amountPerUnitNQT.multiply(new BigInteger(NRS.convertToQNTf(resSupply, decimals)))) + "</td>" +
            ...
            rows += "<tr>" +
            "<td><b>Totals</b></td>" +
            "<td>" + NRS.convertToDAS(totalAmountReserved) + "</td>" +
            "<td>" + NRS.convertToDAS(totalAmountReserved.multiply(new BigInteger(NRS.convertToQNTf(resSupply, decimals)))) + "</td>" +
    ...
    $("#currency_order_modal").on("show.bs.modal", function (e) {
        ...
        var totalDAS = NRS.formatAmount(totalNQT, false, true);
        ...
        $("#currency_order_total").html(totalDAS + " DAS");
    ...
    unitFields.on("change", function() {
        ...
        NRS.sendRequest("getAvailableTo" + NRS.initialCaps(orderType), {
            ...
            rateField.val(rate);
            var amount = NRS.convertToDAS(response.amountNQT);
    ...
    $("#reserve_currency_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrency+", {
            ...
            if (response && !response.errorDescription) {
                ...
                $("#reserve_currency_minReserve_text").html(NRS.formatQuantity(NRS.convertToDAS(minReservePerUnitNQT.multiply(new BigInteger(resSupply))), decimals));
                ...
                $("#reserve_currency_currentReserve_text").html(NRS.formatQuantity(NRS.convertToDAS(currentReservePerUnitNQT.multiply(new BigInteger(resSupply))), decimals));
    ...
    reserveCurrencyAmount.blur(function () {
        ...
        var roundUnitAmountNQT = NRS.convertToNQT(NRS.amountToPrecision(NRS.convertToDAS(unitAmountNQT), decimals));
        $("#reserve_currency_total").val(NRS.convertToDAS(roundUnitAmountNQT));
        reserveCurrencyAmount.val(NRS.convertToDAS(new BigInteger(roundUnitAmountNQT).multiply(new BigInteger(resSupply)).toString()));
    });
    ...
    $("#claim_currency_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrency", {
            ...
            $("#claimRate").html(NRS.formatAmount(currentReservePerUnitNQT) + " [DAS/" + currencyCode + "]");
        });
