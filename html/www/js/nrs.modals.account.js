...
var NRS = (function(NRS, $) {
	...
	NRS.processAccountModalData = function(account) {
		if (account.unconfirmedBalanceNQT == "0") {
			...
			$("#user_info_modal_account_balance").html(NRS.formatAmount(account.unconfirmedBalanceNQT) + " DAS");
		}
        ...
        NRS.sendRequest("getBlockchainTransactions", {
			...
			if (response.transactions && response.transactions.length) {
				...
				for (var i = 0; i < response.transactions.length; i++) {
                    ...
                    var receiving;
					if (/^DAS\-/i.test(String(NRS.userInfoModal.user))) {
		...
		NRS.sendRequest("getDGSGoods", {
			...
			if (response.goods && response.goods.length) {
				for (var i = 0; i < response.goods.length; i++) {
					...
					rows += "<tr><td><a href='#' data-goto-goods='" + NRS.escapeRespStr(good.goods) + "' data-seller='" + NRS.escapeRespStr(NRS.userInfoModal.user) + "'>" + NRS.escapeRespStr(good.name) + "</a></td><td class='numeric'>" + NRS.formatAmount(good.priceNQT, false, false, priceDecimals) + " DAS</td><td class='numeric'>" + NRS.format(good.quantity, false, quantityDecimals) + "</td></tr>";
				}
			}
