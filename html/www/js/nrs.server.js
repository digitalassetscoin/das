...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.sendRequest = function (requestType, data, callback, options) {
        ...
        try {
            var dasFields = [
                ...
                ["priceDAS", "priceNQT"],
                ["refundDAS", "refundNQT"],
                ["discountDAS", "discountNQT"],
                ["phasingQuorumDAS", "phasingQuorum"],
                ["phasingMinBalanceDAS", "phasingMinBalance"],
                ["controlQuorumDAS", "controlQuorum"],
                ["controlMinBalanceDAS", "controlMinBalance"],
                ["controlMaxFeesDAS", "controlMaxFees"],
                ["minBalanceDAS", "minBalance"],
                ["shufflingAmountDAS", "amount"],
                ["monitorAmountDAS", "amount"],
                ["monitorThresholdDAS", "threshold"]
            ];

            for (i = 0; i < dasFields.length; i++) {
                var dasField = dasFields[i][0];
                var nqtField = dasFields[i][1];
                if (dasField in data) {
                    data[nqtField] = NRS.convertToNQT(data[dasField]);
                    delete data[dasField];
                }
            }
        } catch (err) {
            callback({
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToDAS(phasingControl.maxFees)
                    })
                });
                return;
            }
        ...
        if (requestType == "getAccountId") {
            ...
            var accountRS = "";
            if (dasAddress.set(accountId)) {
                accountRS = dasAddress.toString();
            }
            callback({
                "account": accountId,
                "accountRS": accountRS
    ...
    NRS.processAjaxRequest = function (requestType, data, callback, options) {
        ...
        if (options.remoteNode) {
            url = options.remoteNode.getUrl() + "/das";
        } else {
