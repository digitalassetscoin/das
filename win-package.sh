...
PACKAGE=das-client-${VERSION}.zip
echo PACKAGE="${PACKAGE}"

FILES="changelogs classes conf html lib src resource addons"
FILES="${FILES} das.jar dasservice.jar"
...
FILES="${FILES} das.policy dasdesktop.policy DAS_Wallet.url"
...
echo zip
zip -q -X -r ${PACKAGE} das -x \*/.idea/\* \*/.gitignore \*/.git/\* \*/\*.log \*.iml das/conf/das.properties das/conf/logging.properties das/conf/localstorage/\*
rm -rf das
echo done
