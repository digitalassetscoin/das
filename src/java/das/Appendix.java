...
import das.crypto.Crypto;
import das.crypto.EncryptedData;
...
public interface Appendix {
    ...
    interface Prunable {
        ...
        default boolean shouldLoadPrunable(Transaction transaction, boolean includeExpiredPrunable) {
            return Das.getEpochTime() - transaction.getTimestamp() <
    ...
    abstract class AbstractAppendix implements Appendix {
        ...
        abstract void validate(Transaction transaction) throws DasException.ValidationException;

        void validateAtFinish(Transaction transaction) throws DasException.ValidationException {
    ...
    class Message extends AbstractAppendix {
        ...
        private static final Fee MESSAGE_FEE = new Fee.SizeBasedFee(0, Constants.ONE_DAS, 32) {
            @Override
        ...
        Message(ByteBuffer buffer, byte transactionVersion) throws DasException.NotValidException {
            ...
            if (messageLength > 1000) {
                throw new DasException.NotValidException("Invalid arbitrary message length: " + messageLength);
            }
            this.message = new byte[messageLength];
            buffer.get(this.message);
            if (isText && !Arrays.equals(message, Convert.toBytes(Convert.toString(message)))) {
                throw new DasException.NotValidException("Message is not UTF-8 text");
            }
        }
        ...
        @Override
        void validate(Transaction transaction) throws DasException.ValidationException {
            if (Das.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && message.length > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
                throw new DasException.NotValidException("Invalid arbitrary message length: " + message.length);
            }
        }
    ...
    class PrunablePlainMessage extends Appendix.AbstractAppendix implements Prunable {
        ...
        private static final Fee PRUNABLE_MESSAGE_FEE = new Fee.SizeBasedFee(Constants.ONE_DAS/10) {
            @Override
        ...
        void validate(Transaction transaction) throws DasException.ValidationException {
            if (transaction.getMessage() != null) {
                throw new DasException.NotValidException("Cannot have both message and prunable message attachments");
            }
            byte[] msg = getMessage();
            if (msg != null && msg.length > Constants.MAX_PRUNABLE_MESSAGE_LENGTH) {
                throw new DasException.NotValidException("Invalid prunable message length: " + msg.length);
            }
            if (msg == null && Das.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                throw new DasException.NotCurrentlyValidException("Message has been pruned prematurely");
            }
        }

        @Override
        void apply(Transaction transaction, Account senderAccount, Account recipientAccount) {
            if (Das.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME) {
    ...
    abstract class AbstractEncryptedMessage extends AbstractAppendix {
        ...
        private static final Fee ENCRYPTED_MESSAGE_FEE = new Fee.SizeBasedFee(Constants.ONE_DAS, Constants.ONE_DAS, 32) {
            @Override
            public int getSize(TransactionImpl transaction, Appendix appendage) {
        ...
        private AbstractEncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws DasException.NotValidException {
            super(buffer, transactionVersion);
        ...
        @Override
        void validate(Transaction transaction) throws DasException.ValidationException {
            if (Das.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && getEncryptedDataLength() > Constants.MAX_ENCRYPTED_MESSAGE_LENGTH) {
                throw new DasException.NotValidException("Max encrypted message length exceeded");
            }
            if (encryptedData != null) {
                if ((encryptedData.getNonce().length != 32 && encryptedData.getData().length > 0)
                    ...
                    throw new DasException.NotValidException("Invalid nonce length " + encryptedData.getNonce().length);
                }
            }
            if ((getVersion() != 2 && !isCompressed) || (getVersion() == 2 && isCompressed)) {
                throw new DasException.NotValidException("Version mismatch - version " + getVersion() + ", isCompressed " + isCompressed);
            }
        }
    ...
    class PrunableEncryptedMessage extends AbstractAppendix implements Prunable {
        ...
        private static final Fee PRUNABLE_ENCRYPTED_DATA_FEE = new Fee.SizeBasedFee(Constants.ONE_DAS/10) {
            @Override
            public int getSize(TransactionImpl transaction, Appendix appendix) {
                return appendix.getFullSize();
            }
        ...
        void validate(Transaction transaction) throws DasException.ValidationException {
            if (transaction.getEncryptedMessage() != null) {
                throw new DasException.NotValidException("Cannot have both encrypted and prunable encrypted message attachments");
            }
            EncryptedData ed = getEncryptedData();
            if (ed == null && Das.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                throw new DasException.NotCurrentlyValidException("Encrypted message has been pruned prematurely");
            }
            if (ed != null) {
                if (ed.getData().length > Constants.MAX_PRUNABLE_ENCRYPTED_MESSAGE_LENGTH) {
                    throw new DasException.NotValidException(String.format("Message length %d exceeds max prunable encrypted message length %d",
                ...
                if ((ed.getNonce().length != 32 && ed.getData().length > 0)
                    ...
                    throw new DasException.NotValidException("Invalid nonce length " + ed.getNonce().length);
                }
            }
            if (transaction.getRecipientId() == 0) {
                throw new DasException.NotValidException("Encrypted messages cannot be attached to transactions with no recipient");
            }
        }
    ...
    final class UnencryptedPrunableEncryptedMessage extends PrunableEncryptedMessage implements Encryptable {
        ...
        void putMyBytes(ByteBuffer buffer) {
            if (getEncryptedData() == null) {
                throw new DasException.NotYetEncryptedException("Prunable encrypted message not yet encrypted");
            }
    ...
    class EncryptedMessage extends AbstractEncryptedMessage {
        ...
        EncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws DasException.NotValidException {
            super(buffer, transactionVersion);
        }
    ...
    final class UnencryptedEncryptedMessage extends EncryptedMessage implements Encryptable {
        ...
        void putMyBytes(ByteBuffer buffer) {
            if (getEncryptedData() == null) {
                throw new DasException.NotYetEncryptedException("Message not yet encrypted");
            }
            super.putMyBytes(buffer);
        }
    ...
    class EncryptToSelfMessage extends AbstractEncryptedMessage {
        ...
        EncryptToSelfMessage(ByteBuffer buffer, byte transactionVersion) throws DasException.NotValidException {
            super(buffer, transactionVersion);
        }
    ...
    final class PublicKeyAnnouncement extends AbstractAppendix {
        ...
        void validate(Transaction transaction) throws DasException.ValidationException {
            if (transaction.getRecipientId() == 0) {
                throw new DasException.NotValidException("PublicKeyAnnouncement cannot be attached to transactions with no recipient");
            }
            if (!Crypto.isCanonicalPublicKey(publicKey)) {
                throw new DasException.NotValidException("Invalid recipient public key: " + Convert.toHexString(publicKey));
            }
            long recipientId = transaction.getRecipientId();
            if (Account.getId(this.publicKey) != recipientId) {
                throw new DasException.NotValidException("Announced public key does not match recipient accountId");
            }
            ...
            if (recipientPublicKey != null && ! Arrays.equals(publicKey, recipientPublicKey)) {
                throw new DasException.NotCurrentlyValidException("A different public key for this account has already been announced");
            }
        }
    ...
    final class Phasing extends AbstractAppendix {
        ...
        private static final Fee PHASING_FEE = (transaction, appendage) -> {
            ...
            if (!phasing.params.getVoteWeighting().isBalanceIndependent()) {
                fee += 20 * Constants.NUMERATOR_FEE * Constants.ONE_DAS / Constants.DENOMINATOR_FEE;
            } else {
                fee += Constants.NUMERATOR_FEE * Constants.ONE_DAS / Constants.DENOMINATOR_FEE;
            }
            if (phasing.hashedSecret.length > 0) {
                fee += (1 + (phasing.hashedSecret.length - 1) / 32) * Constants.NUMERATOR_FEE * Constants.ONE_DAS / Constants.DENOMINATOR_FEE;
            }
            fee += Constants.NUMERATOR_FEE * Constants.ONE_DAS * phasing.linkedFullHashes.length / Constants.DENOMINATOR_FEE;
            return fee;
        };
        ...
        void validate(Transaction transaction) throws DasException.ValidationException {
            params.validate();
            int currentHeight = Das.getBlockchain().getHeight();
            if (params.getVoteWeighting().getVotingModel() == VoteWeighting.VotingModel.TRANSACTION) {
                if (linkedFullHashes.length == 0 || linkedFullHashes.length > Constants.MAX_PHASING_LINKED_TRANSACTIONS) {
                    throw new DasException.NotValidException("Invalid number of linkedFullHashes " + linkedFullHashes.length);
                }
                Set<Long> linkedTransactionIds = new HashSet<>(linkedFullHashes.length);
                for (byte[] hash : linkedFullHashes) {
                    if (Convert.emptyToNull(hash) == null || hash.length != 32) {
                        throw new DasException.NotValidException("Invalid linkedFullHash " + Convert.toHexString(hash));
                    }
                    if (Das.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK) {
                        if (!linkedTransactionIds.add(Convert.fullHashToId(hash))) {
                            throw new DasException.NotValidException("Duplicate linked transaction ids");
                        }
                    }
                    ...
                    if (linkedTransaction != null) {
                        if (transaction.getTimestamp() - linkedTransaction.getTimestamp() > Constants.MAX_REFERENCED_TRANSACTION_TIMESPAN) {
                            throw new DasException.NotValidException("Linked transaction cannot be more than 60 days older than the phased transaction");
                        }
                        if (linkedTransaction.getPhasing() != null) {
                            throw new DasException.NotCurrentlyValidException("Cannot link to an already existing phased transaction");
                        }
                    }
                }
                if (params.getQuorum() > linkedFullHashes.length) {
                    throw new DasException.NotValidException("Quorum of " + params.getQuorum() + " cannot be achieved in by-transaction voting with "
                            + linkedFullHashes.length + " linked full hashes only");
                }
            } else {
                if (linkedFullHashes.length != 0) {
                    throw new DasException.NotValidException("LinkedFullHashes can only be used with VotingModel.TRANSACTION");
                }
            }

            if (params.getVoteWeighting().getVotingModel() == VoteWeighting.VotingModel.HASH) {
                if (params.getQuorum() != 1) {
                    throw new DasException.NotValidException("Quorum must be 1 for by-hash voting");
                }
                if (hashedSecret.length == 0 || hashedSecret.length > Byte.MAX_VALUE) {
                    throw new DasException.NotValidException("Invalid hashedSecret " + Convert.toHexString(hashedSecret));
                }
                if (PhasingPoll.getHashFunction(algorithm) == null) {
                    throw new DasException.NotValidException("Invalid hashedSecretAlgorithm " + algorithm);
                }
            } else {
                if (hashedSecret.length != 0) {
                    throw new DasException.NotValidException("HashedSecret can only be used with VotingModel.HASH");
                }
                if (algorithm != 0) {
                    throw new DasException.NotValidException("HashedSecretAlgorithm can only be used with VotingModel.HASH");
                }
            }
            ...
            if (finishHeight <= currentHeight + (params.getVoteWeighting().acceptsVotes() ? 2 : 1)
                ...
                throw new DasException.NotCurrentlyValidException("Invalid finish height " + finishHeight);
            }
        }
        ...
        void countVotes(TransactionImpl transaction) {
            if (Das.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && PhasingPoll.getResult(transaction.getId()) != null) {
                return;
            }
            ...
            if (result >= poll.getQuorum()) {
                if (!transaction.attachmentIsDuplicate(duplicates, false)) {
                    try {
                        release(transaction);
                        poll.finish(result);
                        Logger.logDebugMessage("Early finish of transaction " + transaction.getStringId() + " at height " + Das.getBlockchain().getHeight());
                    } catch (RuntimeException e) {
                        Logger.logErrorMessage("Failed to release phased transaction " + transaction.getJSONObject().toJSONString(), e);
                    }
                } else {
                    Logger.logDebugMessage("At height " + Das.getBlockchain().getHeight() + " phased transaction " + transaction.getStringId()
