...
import das.db.VersionedPrunableDbTable;
import das.db.VersionedValuesDbTable;
...
public class TaggedData {
        ...
        protected void prune() {
            if (Constants.ENABLE_PRUNING) {
                try (Connection con = db.getConnection();
                    ...
                    int expiration = Das.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME;
    ...
    public static final class Tag {
        ...
        private static void add(TaggedData taggedData) {
            for (String tagValue : taggedData.getParsedTags()) {
                ...
                if (tag == null) {
                    tag = new Tag(tagValue, Das.getBlockchain().getHeight());
                }
                tag.count += 1;
                tagTable.insert(tag);
            }
    ...
    public TaggedData(Transaction transaction, Attachment.TaggedDataAttachment attachment) {
        this(transaction, attachment, Das.getBlockchain().getLastBlockTimestamp(), Das.getBlockchain().getHeight());
    }
    ...
    static void add(TransactionImpl transaction, Attachment.TaggedDataUpload attachment) {
        if (Das.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME && attachment.getData() != null) {
    ...
    static void extend(Transaction transaction, Attachment.TaggedDataExtend attachment) {
        ...
        if (Das.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME < timestamp.timestamp) {
            ...
            if (taggedData != null) {
                ...
                taggedData.blockTimestamp = Das.getBlockchain().getLastBlockTimestamp();
                taggedData.height = Das.getBlockchain().getHeight();
                taggedDataTable.insert(taggedData);
            }
        }
    }
