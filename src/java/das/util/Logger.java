...
/**
 * Handle logging for the Das node server
 */
public final class Logger {
     ...
     * The das/conf/logging-default.properties and das/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "das.util.DasLogManager");
        if (!(LogManager.getLogManager() instanceof DasLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("das.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Das.loadProperties(loggingProperties, "logging-default.properties", true);
                Das.loadProperties(loggingProperties, "logging.properties", false);
                Das.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(das.Das.class);
        enableStackTraces = Das.getBooleanProperty("das.enableStackTraces");
        enableLogTraceback = Das.getBooleanProperty("das.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof DasLogManager) {
            ((DasLogManager) LogManager.getLogManager()).dasShutdown();
        }
    }
