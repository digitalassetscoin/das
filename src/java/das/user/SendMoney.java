...
import static das.user.JSONResponses.NOTIFY_OF_ACCEPTED_TRANSACTION;
...
public final class SendMoney extends UserServlet.UserRequestHandler {
    ...
    @Override
    JSONStreamAware processRequest(HttpServletRequest req, User user) throws DasException.ValidationException, IOException {
        if (user.getSecretPhrase() == null) {
            return null;
        }

        String recipientValue = req.getParameter("recipient");
        String amountValue = req.getParameter("amountDAS");
        String feeValue = req.getParameter("feeDAS");
        ...
        try {
            ...
            amountNQT = Convert.parseDAS(amountValue.trim());
            feeNQT = Convert.parseDAS(feeValue.trim());
            ...
            response.put("amountDAS", amountValue);
            response.put("feeDAS", feeValue);
        ...
        } else if (feeNQT < Constants.ONE_DAS || feeNQT > Constants.MAX_BALANCE_NQT) {

            JSONObject response = new JSONObject();
            response.put("response", "notifyOfIncorrectTransaction");
            response.put("message", "\"Fee\" must be at least 1 DAS!");
            response.put("recipient", recipientValue);
            response.put("amountDAS", amountValue);
            response.put("feeDAS", feeValue);
            response.put("deadline", deadlineValue);
        ...
        if (account == null || Math.addExact(amountNQT, feeNQT) > account.getUnconfirmedBalanceNQT()) {
            ...
            final Transaction transaction = Das.newTransactionBuilder(user.getPublicKey(),
