...
import das.db.DbClause;
import das.db.DbIterator;
import das.db.DbKey;
import das.db.EntityDbTable;
...
final class TransactionProcessorImpl implements TransactionProcessor {
    ...
    private static final boolean enableTransactionRebroadcasting = Das.getBooleanProperty("das.enableTransactionRebroadcasting");
    private static final boolean testUnconfirmedTransactions = Das.getBooleanProperty("das.testUnconfirmedTransactions");
    private static final int maxUnconfirmedTransactions;
    static {
        int n = Das.getIntProperty("das.maxUnconfirmedTransactions");
    ...
    private final Runnable removeUnconfirmedTransactionsThread = () -> {
        ...
        try {
            try {
                if (Das.getBlockchainProcessor().isDownloading() && ! testUnconfirmedTransactions) {
                    return;
                }
                ...
                try (DbIterator<UnconfirmedTransaction> iterator = unconfirmedTransactionTable.getManyBy(
                        new DbClause.IntClause("expiration", DbClause.Op.LT, Das.getEpochTime()), 0, -1, "")) {
    ...
    private final Runnable processTransactionsThread = () -> {
        ...
        try {
            try {
                ...
                } catch (DasException.ValidationException|RuntimeException e) {
                    peer.blacklist(e);
                }
            } catch (Exception e) {
    ...
    @Override
    public void broadcast(Transaction transaction) throws DasException.ValidationException {
        BlockchainImpl.getInstance().writeLock();
        try {
    ...
    @Override
    public void processPeerTransactions(JSONObject request) throws DasException.ValidationException {
    ...
    void processWaitingTransactions() {
        ...
        try {
            if (waitingTransactions.size() > 0) {
                int currentTime = Das.getEpochTime();
                ...
                while (iterator.hasNext()) {
                    ...
                    } catch (DasException.ExistingTransactionException e) {
                        iterator.remove();
                    } catch (DasException.NotCurrentlyValidException e) {
    ...
    private void processPeerTransactions(JSONArray transactionsData) throws DasException.NotValidException {
        if (Das.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
            return;
        }
        ...
        for (Object transactionData : transactionsData) {
            ...
            } catch (DasException.NotCurrentlyValidException ignore) {
            } catch (DasException.ValidationException|RuntimeException e) {
        ...
        if (!exceptions.isEmpty()) {
            throw new DasException.NotValidException("Peer sends invalid transactions: " + exceptions.toString());
        }
    }

    private void processTransaction(UnconfirmedTransaction unconfirmedTransaction) throws DasException.ValidationException {
        TransactionImpl transaction = unconfirmedTransaction.getTransaction();
        int curTime = Das.getEpochTime();
        if (transaction.getTimestamp() > curTime + Constants.MAX_TIMEDRIFT || transaction.getExpiration() < curTime) {
            throw new DasException.NotCurrentlyValidException("Invalid transaction timestamp");
        }
        if (transaction.getVersion() < 1) {
            throw new DasException.NotValidException("Invalid transaction version");
        }
        if (transaction.getId() == 0L) {
            throw new DasException.NotValidException("Invalid transaction id 0");
        }
        ...
        try {
            try {
                Db.db.beginTransaction();
                if (Das.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
                    throw new DasException.NotCurrentlyValidException("Blockchain not ready to accept transactions");
                }

                if (getUnconfirmedTransaction(transaction.getDbKey()) != null || TransactionDb.hasTransaction(transaction.getId())) {
                    throw new DasException.ExistingTransactionException("Transaction already processed");
                }

                if (! transaction.verifySignature()) {
                    if (Account.getAccount(transaction.getSenderId()) != null) {
                        throw new DasException.NotValidException("Transaction signature verification failed");
                    } else {
                        throw new DasException.NotCurrentlyValidException("Unknown transaction sender");
                    }
                }

                if (! transaction.applyUnconfirmed()) {
                    throw new DasException.InsufficientBalanceException("Insufficient balance");
                }

                if (transaction.isUnconfirmedDuplicate(unconfirmedDuplicates)) {
                    throw new DasException.NotCurrentlyValidException("Duplicate unconfirmed transaction");
                }
     ...
     * @throws  DasException.NotValidException    Transaction is not valid
     */
    @Override
    public List<Transaction> restorePrunableData(JSONArray transactions) throws DasException.NotValidException {
