...
import das.util.CountingInputReader;
import das.util.CountingInputStream;
import das.util.CountingOutputWriter;
...
final class PeerImpl implements Peer {
    ...
    void setVersion(String version) {
        ...
        isOldVersion = false;
        if (Das.APPLICATION.equals(application)) {
    ...
    public int getWeight() {
        ...
        if (hallmarkBalance == -1 || hallmarkBalanceHeight < Das.getBlockchain().getHeight() - 60) {
            long accountId = hallmark.getAccountId();
            Account account = Account.getAccount(accountId);
            hallmarkBalance = account == null ? 0 : account.getBalanceNQT();
            hallmarkBalanceHeight = Das.getBlockchain().getHeight();
        }
        return (int)(adjustedWeight * (hallmarkBalance / Constants.ONE_DAS) / Constants.MAX_BALANCE_DAS);
    }
    ...
    public void blacklist(Exception cause) {
        if (cause instanceof DasException.NotCurrentlyValidException || cause instanceof BlockchainProcessor.BlockOutOfOrderException
    ...
    public void blacklist(String cause) {
        blacklistingTime = Das.getEpochTime();
    ...
    public JSONObject send(final JSONStreamAware request, int maxResponseSize) {
        ...
        try {
            ...
            if (useWebSocket && !webSocket.isOpen())
                useWebSocket = webSocket.startClient(URI.create("ws://" + host + ":" + getPort() + "/das"));
            //
            ...
            if (useWebSocket) {
                ...
                if (maxResponseSize > 0) {
                    ...
                    if (wsResponse.length() > maxResponseSize)
                        throw new DasException.DasIOException("Maximum size exceeded: " + wsResponse.length());
                ...
                //
                URL url = new URL("http://" + host + ":" + getPort() + "/das");
                if (communicationLoggingMask != 0)
        ...
        } catch (DasException.DasIOException e) {
            blacklist(e);
            if (connection != null) {
                connection.disconnect();
            }
    ...
    void connect() {
        lastConnectAttempt = Das.getEpochTime();
        try {
    ...
    boolean analyzeHallmark(final String hallmarkString) {
        ...
        try {
            ...
            for (PeerImpl peer : groupedPeers) {
                peer.adjustedWeight = Constants.MAX_BALANCE_DAS * peer.getHallmarkWeight(mostRecentDate) / totalWeight;
