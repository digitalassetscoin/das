...
public final class ShufflingParticipant {
    ...
    void setData(byte[][] data, int timestamp) {
        if (data != null && Das.getEpochTime() - timestamp < Constants.MAX_PRUNABLE_LIFETIME && getData() == null) {
            shufflingDataTable.insert(new ShufflingData(shufflingId, accountId, data, timestamp, Das.getBlockchain().getHeight()));
        }
    }
