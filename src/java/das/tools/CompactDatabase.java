 ...
 *
 *   java -cp "classes:lib/*:conf" das.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Ddas.runtime.mode=desktop das.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Das properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "das.testDb" : "das.db";
        String dbType = Das.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Das.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Das.getDbDir(Das.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Das.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Das.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Das.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "das.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "das.mv.db");
            if (!dbFile.exists()) {
