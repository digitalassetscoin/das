...
public final class DebugTrace {
    ...
    static final String QUOTE = Das.getStringProperty("das.debugTraceQuote", "\"");
    static final String SEPARATOR = Das.getStringProperty("das.debugTraceSeparator", "\t");
    static final boolean LOG_UNCONFIRMED = Das.getBooleanProperty("das.debugLogUnconfirmed");

    static void init() {
        List<String> accountIdStrings = Das.getStringListProperty("das.debugTraceAccounts");
        String logName = Das.getStringProperty("das.debugTraceLog");
        ...
        Das.getBlockchainProcessor().addListener(block -> debugTrace.resetLog(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public static DebugTrace addDebugTrace(Set<Long> accountIds, String logName) {
        ...
        Das.getBlockchainProcessor().addListener(debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
        Das.getBlockchainProcessor().addListener(debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
        Das.getTransactionProcessor().addListener(transactions -> debugTrace.traceRelease(transactions.get(0)), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
    ...
    private void traceShufflingCancel(Shuffling shuffling) {
        ...
        if (blamedAccountId != 0 && include(blamedAccountId)) {
            ...
            long generatorId = Das.getBlockchain().getLastBlock().getGeneratorId();
            if (include(generatorId)) {
                Map<String,String> generatorMap = getValues(generatorId, false);
                generatorMap.put("generation fee", String.valueOf(fee));
                generatorMap.put("event", "shuffling blame");
    ...
    private Map<String,String> lessorGuaranteedBalance(Account account, long lesseeId) {
        ...
        map.put("timestamp", String.valueOf(Das.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(Das.getBlockchain().getHeight()));
    ...
    private Map<String,String> getValues(long accountId, Shuffling shuffling, boolean isRecipient) {
        ...
        if (shuffling.getHoldingType() == HoldingType.DAS) {
    ...
    private Map<String,String> getValues(long accountId, Block block) {
        ...
        if (block.getHeight() > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                if (include(previousGeneratorId)) {
                    ...
                    map.put("effective balance", String.valueOf(Account.getAccount(previousGeneratorId).getEffectiveBalanceDAS()));
        ...
        Map<String,String> map = getValues(accountId, false);
        map.put("effective balance", String.valueOf(Account.getAccount(accountId).getEffectiveBalanceDAS()));
