...
public final class Poll extends AbstractPoll {
    ...
    private static final boolean isPollsProcessing = Das.getBooleanProperty("das.processPolls");
    ...
    public static DbIterator<Poll> getActivePolls(int from, int to) {
        return pollTable.getManyBy(new DbClause.IntClause("finish_height", DbClause.Op.GT, Das.getBlockchain().getHeight()), from, to);
    }
    ...
    public static DbIterator<Poll> getPollsByAccount(long accountId, boolean includeFinished, boolean finishedOnly, int from, int to) {
        ...
        if (finishedOnly) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.LTE, Das.getBlockchain().getHeight()));
        } else if (!includeFinished) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.GT, Das.getBlockchain().getHeight()));
        }
    ...
    public static DbIterator<Poll> searchPolls(String query, boolean includeFinished, int from, int to) {
        DbClause dbClause = includeFinished ? DbClause.EMPTY_CLAUSE : new DbClause.IntClause("finish_height", DbClause.Op.GT, Das.getBlockchain().getHeight());
    ...
    private Poll(Transaction transaction, Attachment.MessagingPollCreation attachment) {
        ...
        this.timestamp = Das.getBlockchain().getLastBlockTimestamp();
    }
    ...
    public boolean isFinished() {
        return finishHeight <= Das.getBlockchain().getHeight();
    }

    private List<OptionResult> countResults(VoteWeighting voteWeighting) {
        int countHeight = Math.min(finishHeight, Das.getBlockchain().getHeight());
        if (countHeight < Das.getBlockchainProcessor().getMinRollbackHeight()) {
            return null;
        }
