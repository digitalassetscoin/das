...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Das.getBlockchainProcessor().isScanning() ?
                    Math.max(Das.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
                    : Das.getBlockchainProcessor().getMinRollbackHeight();
    ...
    private boolean doesNotExceed(int height) {
        return Das.getBlockchain().getHeight() <= height && ! (isPersistent() && Das.getBlockchainProcessor().isScanning());
    }
