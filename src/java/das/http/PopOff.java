...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            Das.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = Das.getBlockchainProcessor().popOffTo(Das.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = Das.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            Das.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> Das.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }
