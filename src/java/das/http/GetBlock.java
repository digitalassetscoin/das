...
import static das.http.JSONResponses.INCORRECT_BLOCK;
import static das.http.JSONResponses.INCORRECT_HEIGHT;
import static das.http.JSONResponses.INCORRECT_TIMESTAMP;
import static das.http.JSONResponses.UNKNOWN_BLOCK;
...
public final class GetBlock extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        if (blockValue != null) {
            try {
                blockData = Das.getBlockchain().getBlock(Convert.parseUnsignedLong(blockValue));
            ...
            try {
                ...
                blockData = Das.getBlockchain().getBlockAtHeight(height);
            } catch (RuntimeException e) {
                return INCORRECT_HEIGHT;
            }
            ...
            try {
                ...
                blockData = Das.getBlockchain().getLastBlock(timestamp);
            } catch (RuntimeException e) {
                return INCORRECT_TIMESTAMP;
            }
        } else {
            blockData = Das.getBlockchain().getLastBlock();
        }
