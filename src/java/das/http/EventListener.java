...
import das.AccountLedger;
import das.AccountLedger.LedgerEntry;
import das.Block;
import das.BlockchainProcessor;
import das.Db;
import das.Das;
import das.Transaction;
import das.TransactionProcessor;
import das.db.TransactionalDb;
import das.peer.Peer;
 ...
 * has not been received within das.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by das.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Das.getIntProperty("das.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Das.getIntProperty("das.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Das.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Das.getTransactionProcessor();
    ...
    /** Das event listeners */
    private final List<DasEventListener> dasEventListeners = new ArrayList<>();
     ...
     *
     * Das event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Das event registrations
     ...
     *
     * @param   eventRegistrations      Das event registrations
     * @throws  EventListenerException  Invalid Das event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<DasEventListener> it = dasEventListeners.iterator();
                while (it.hasNext()) {
                    DasEventListener listener = it.next();
                ...
                if (addListener) {
                    DasEventListener listener = new DasEventListener(event);
                    listener.addListener();
                    dasEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            dasEventListeners.forEach(DasEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * Das event listener
     */
    private class DasEventListener {

        /** Event handler */
        private final DasEventHandler eventHandler;

        /**
         * Create the Das event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public DasEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Das listener for this event
         */
        ...
        /**
         * Remove the Das listener for this event
         */
        ...
        /**
         * Return the hash code for this Das event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Das events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof DasEventListener) &&
                    eventHandler.equals(((DasEventListener)obj).eventHandler));
        }

        /**
         * Das listener event handler
         */
        private abstract class DasEventHandler {
            ...
            /**
             * Create the Das event handler
             *
             * @param   eventRegistration   Event registration
             */
            public DasEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof DasEventHandler) &&
                                        owner == ((DasEventHandler)obj).owner &&
                                        accountId == ((DasEventHandler)obj).accountId &&
                                        event == ((DasEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends DasEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends DasEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends DasEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends DasEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Das listener event
        ...
        /**
         * Return the Das listener event
         *
         * @return                  Das listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
