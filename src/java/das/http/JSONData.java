...
import das.AccountRestrictions;
import das.Alias;
import das.Appendix;
import das.Asset;
import das.AssetDelete;
import das.AssetDividend;
import das.AssetTransfer;
import das.Attachment;
import das.Block;
import das.Constants;
import das.Currency;
import das.CurrencyExchangeOffer;
import das.CurrencyFounder;
import das.CurrencyTransfer;
import das.CurrencyType;
import das.DigitalGoodsStore;
import das.Exchange;
import das.ExchangeRequest;
import das.FundingMonitor;
import das.Generator;
import das.HoldingType;
import das.MonetarySystem;
import das.Das;
import das.Order;
import das.PhasingPoll;
import das.PhasingVote;
import das.Poll;
import das.PrunableMessage;
import das.Shuffler;
import das.Shuffling;
import das.ShufflingParticipant;
import das.TaggedData;
import das.Token;
import das.Trade;
import das.Transaction;
import das.Vote;
import das.VoteWeighting;
import das.crypto.Crypto;
import das.crypto.EncryptedData;
import das.db.DbIterator;
import das.peer.Hallmark;
...
public final class JSONData {
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        return accountBalance(account, includeEffectiveBalance, Das.getBlockchain().getHeight());
    }
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance, int height) {
        ...
        if (account == null) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceDAS", "0");
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceDAS", account.getEffectiveBalanceDAS(height));
    ...
    static JSONObject lessor(Account account, boolean includeEffectiveBalance) {
        ...
        if (accountLease.getCurrentLesseeId() != 0) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceDAS", String.valueOf(account.getGuaranteedBalanceNQT() / Constants.ONE_DAS));
            }
        }
    ...
    static JSONObject shuffling(Shuffling shuffling, boolean includeHoldingInfo) {
        ...
        if (includeHoldingInfo && shuffling.getHoldingType() != HoldingType.DAS) {
        ...
        if (includeExecutedPhased) {
            ...
            try (DbIterator<PhasingPoll.PhasingPollResult> phasingPollResults = PhasingPoll.getApproved(block.getHeight())) {
                for (PhasingPoll.PhasingPollResult phasingPollResult : phasingPollResults) {
                    ...
                    if (includeTransactions) {
                        phasedTransactions.add(transaction(Das.getBlockchain().getTransaction(phasedTransactionId)));
                    } else {
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Das.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject transaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("confirmations", Das.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", Das.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
    ...
    static void ledgerEntry(JSONObject json, LedgerEntry entry, boolean includeTransactions, boolean includeHoldingInfo) {
        ...
        if (includeTransactions && entry.getEvent().isTransaction()) {
            Transaction transaction = Das.getBlockchain().getTransaction(entry.getEventId());
