...
/**
 * Issue a currency on the DAS blockchain
 * <p>
 * A currency is the basic block of the DAS Monetary System it can be exchanged with DAS, transferred between accounts,
 ...
 * For {@link das.CurrencyType#RESERVABLE} currency
 * <li>minReservePerUnitNQT - the minimum DAS value per unit to allow the currency to become active
 * For {@link das.CurrencyType#RESERVABLE} currency
 ...
 * For {@link das.CurrencyType#MINTABLE} currency
 * <li>maxDifficulty - for mint-able currency, the exponent of the final difficulty.
 * For {@link das.CurrencyType#MINTABLE} currency
 * <li>algorithm - the hashing {@link das.crypto.HashFunction algorithm} used for minting.
 * For {@link das.CurrencyType#MINTABLE} currency
 * </ul>
 * <p>
 * Constraints
 * <ul>
 * <li>A given currency can not be neither {@link das.CurrencyType#EXCHANGEABLE} nor {@link das.CurrencyType#CLAIMABLE}.<br>
 * <li>A {@link das.CurrencyType#RESERVABLE} currency becomes active once the blockchain height reaches the currency issuance height.<br>
 ...
 * When a {@link das.CurrencyType#RESERVABLE} becomes active, in case it is {@link das.CurrencyType#CLAIMABLE} the DAS used for
 * reserving the currency are locked until they are claimed back.
 * When a {@link das.CurrencyType#RESERVABLE} becomes active, in case it is non {@link das.CurrencyType#CLAIMABLE} the DAS used for
 * reserving the currency are sent to the issuer account as crowd funding.
 * <li>When issuing a {@link das.CurrencyType#MINTABLE} currency, the number of units per {@link das.http.CurrencyMint} cannot exceed 0.01% of the
 ...
 * difficulty increases linearly with the number units minted per {@link das.http.CurrencyMint}<br>
 * </ul>
 *
 * @see das.CurrencyType
 * @see das.crypto.HashFunction
 */
