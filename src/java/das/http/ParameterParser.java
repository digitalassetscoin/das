...
import das.Currency;
import das.CurrencyBuyOffer;
import das.CurrencySellOffer;
...
import static das.http.JSONResponses.*;
...
public final class ParameterParser {
    ...
    public static int getNumberOfConfirmations(HttpServletRequest req) throws ParameterException {
        return getInt(req, "numberOfConfirmations", 0, Das.getBlockchain().getHeight(), false);
    }
    ...
    public static int getHeight(HttpServletRequest req) throws ParameterException {
        ...
        if (heightValue != null) {
            try {
                ...
                if (height < 0 || height > Das.getBlockchain().getHeight()) {
    ...
    public static long getHoldingId(HttpServletRequest req, HoldingType holdingType) throws ParameterException {
        long holdingId = ParameterParser.getUnsignedLong(req, "holding", holdingType != HoldingType.DAS);
        if (holdingType == HoldingType.DAS && holdingId != 0) {
            throw new ParameterException(JSONResponses.incorrect("holding", "holding id should not be specified if holdingType is DAS"));
        }
        return holdingId;
    }
    ...
    public static Transaction.Builder parseTransaction(String transactionJSON, String transactionBytes, String prunableAttachmentJSON) throws ParameterException {
        ...
        if (transactionJSON != null) {
            try {
                ...
                return Das.newTransactionBuilder(json);
            } catch (DasException.ValidationException | RuntimeException | ParseException e) {
            ...
            try {
                ...
                return Das.newTransactionBuilder(bytes, prunableAttachments);
            } catch (DasException.ValidationException|RuntimeException | ParseException e) {
                Logger.logDebugMessage(e.getMessage(), e);
                JSONObject response = new JSONObject();
    ...
    public static Attachment.TaggedDataUpload getTaggedData(HttpServletRequest req) throws ParameterException, DasException.NotValidException {
