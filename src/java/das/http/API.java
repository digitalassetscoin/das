...
import static das.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static das.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
import static das.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
...
public final class API {
    ...
    public static final String adminPassword = Das.getStringProperty("das.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Das.getIntProperty("das.maxAPIRecords");
    static final boolean enableAPIUPnP = Das.getBooleanProperty("das.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Das.getIntProperty("das.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Das.getBooleanProperty("das.apiServerCORS");
    ...
    static {
        List<String> disabled = new ArrayList<>(Das.getStringListProperty("das.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Das.getStringListProperty("das.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Das.getStringListProperty("das.allowedBotHosts");
        ...
        boolean enableAPIServer = Das.getBooleanProperty("das.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Das.getIntProperty("das.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Das.getIntProperty("das.apiServerSSLPort");
            final String host = Das.getStringProperty("das.apiServerHost");
            disableAdminPassword = Das.getBooleanProperty("das.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Das.getBooleanProperty("das.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Das.getUserHomeDir()).resolve(Paths.get(Das.getStringProperty("das.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Das.getStringProperty("das.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(Das.getStringProperty("das.keyStoreType"));
                List<String> ciphers = Das.getStringListProperty("das.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Das.getStringProperty("das.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Das.getStringProperty("das.apiWelcomeFile")});
            }

            String javadocResourceBase = Das.getStringProperty("das.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/das");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Das.getIntProperty("das.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/das-proxy");
            ...
            if (!Das.getBooleanProperty("das.enableAPIServerGZIPFilter")) {
                gzipHandler.setExcludedPaths("/das", "/das-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(das.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (Das.getBooleanProperty("das.apiFrameOptionsSameOrigin")) {
