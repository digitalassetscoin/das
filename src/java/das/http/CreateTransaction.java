...
import static das.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static das.http.JSONResponses.INCORRECT_DEADLINE;
import static das.http.JSONResponses.INCORRECT_EC_BLOCK;
import static das.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static das.http.JSONResponses.INCORRECT_WHITELIST;
import static das.http.JSONResponses.MISSING_DEADLINE;
import static das.http.JSONResponses.MISSING_SECRET_PHRASE;
import static das.http.JSONResponses.NOT_ENOUGH_FUNDS;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                Das.getBlockchain().getHeight() + 1,
                Das.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountNQT, Attachment attachment) throws DasException {
        ...
        if (ecBlockId != 0 && ecBlockId != Das.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = Das.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = Das.newTransactionBuilder(publicKey, amountNQT, feeNQT,
            ...
            } catch (DasException.NotYetEncryptedException ignore) {}
        ...
        } catch (DasException.NotYetEnabledException e) {
