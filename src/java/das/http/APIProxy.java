...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Das.getBooleanProperty("das.enableAPIProxy") && API.openAPIPort == 0 && API.openAPISSLPort == 0);
    private static final int blacklistingPeriod = Das.getIntProperty("das.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = Das.getStringProperty("das.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Das.getBlockchainProcessor().isDownloading());
    }

    void blacklistHost(String host) {
        blacklistedPeers.put(host, Das.getEpochTime() + blacklistingPeriod);
