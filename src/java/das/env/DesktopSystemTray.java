...
public class DesktopSystemTray {
    ...
    void createAndShowGUI() {
        ...
        imageIcon = new ImageIcon("html/www/img/das-icon-32x32.png", "tray icon");
        ...
        if (!Das.isDesktopApplicationEnabled()) {
        ...
        showDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("dasdesktop.DesktopApplication").getMethod("launch").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("dasdesktop.DesktopApplication failed to launch", exception);
            }
        });

        refreshDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("dasdesktop.DesktopApplication").getMethod("refresh").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("dasdesktop.DesktopApplication failed to refresh", exception);
            }
        });
    ...
    private void displayStatus() {
        ...
        addDataRow(statusPanel, "Application", Das.APPLICATION);
        addDataRow(statusPanel, "Version", Das.VERSION);
        ...
        addDataRow(statusPanel, "User folder", String.valueOf(Paths.get(Das.getUserHomeDir()).toAbsolutePath()));
        ...
        if (lastBlock != null) {
            ...
            addDataRow(statusPanel, "Seconds passed", String.valueOf(Das.getEpochTime() - lastBlock.getTimestamp()));
        ...
        addDataRow(statusPanel, "Process id", Das.getProcessId());
        addEmptyRow(statusPanel);
        ...
        if (statusDialog == null || !statusDialog.isVisible()) {
            ...
            statusDialog = pane.createDialog(wrapper, "DAS Server Status");
