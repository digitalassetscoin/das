...
jar/exe/dmg package, or run "java -jar das-client.jar" to start the
installer. See https://bitbucket.org/JeanLucPicard/das/issues/283 for more
details about the IzPack installer. After installation, use the shortcuts or
desktop icons to start the Das server.


Using the das-client.zip package:

Unpack the das-client.zip package and open a shell in the resulting das
...
the das_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.das for
configuration files, logs, and the das_db database. Unlike run.sh, start.sh
...
message "Das server 1.x.x started successfully" in the console log. If run in
...
where the Das UI should now be available.
...
to the Das installation directory, as the use of special characters may result
...
conf/das-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
das-default.properties directly, but create a new conf/das.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from das-default.properties, the
settings in das.properties override those in das-default.properties. This way,
when upgrading the software, you can safely overwrite das-default.properties
with the updated file from the new package, while your customizations remain
safe in the das.properties file.


How to contribute?

There are many ways to contribute to Das. Here are some examples:
...
The Das software is a client-server application. It consists of a java server
...
requests which other Das client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
das_db directory. When upgrading, you should not delete the old das_db
...
das_db, except that it will take some extra time to download the blockchain
from scratch.

The default Das client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file das.log, which gets
overwritten at restart. In case of an error, the das.log file may contain
...
das.jar file using the enclosed jar.sh script, and then das.jar should be
