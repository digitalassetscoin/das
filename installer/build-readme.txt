Building the DAS installer

Note: all pathes are relative to the git repository folder (das-private)
...
Test X11 installer: java -jar das-client-<version>.jar
Test console installer: java -jar das-client-<version>.jar -console
...
Copy the content of Java JRE folder (C:\Program Files\Java\jre<version>) into the jre folder under your das-private git repository preserving the folder structure
...
Test the installer using Java: java -jar das-client-<version>.jar
Test the windows executable: das-client-<version>.exe
...
das.jar is now self executing, the full classpath is embedded into the jar file manifest automatically by jar.sh
